    ...
    */
    async getMenuItems() {
        ...
        data.forEach(element => {
            const menu = this.getChildrenName(element.children);
            if (menu) {
                menuItems.push(menu);
            }
        });
    ...
    getChildrenName(children: MenuItem[]): MenuItem | null | undefined {
        if (children && children.length && children.length == 1) {
            return children.find(() => true);
        } else if (children && children.length && children.length > 1) {
            return this.getChildrenName(children);
        } else {
            return null;
        }
    }
